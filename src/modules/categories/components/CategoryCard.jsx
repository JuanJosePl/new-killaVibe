...
import { 
  formatProductCount, 
  getCategoryUrl, 
  getCategoryImage // Importante recuperar este helper
...
const CategoryCard = ({
  ...
  // 1. Recuperamos la imagen original usando tu helper inicial
  // Esto garantiza que si no hay productos, se use la imagen correcta (hero o thumbnail)
  ...
  // Si hay productos, buscamos la imagen del producto actual del slider
  const productThumb = hasProducts 
    ? (categoryProducts[currentImageIndex]?.images?.[0]?.url || categoryProducts[currentImageIndex]?.image)
    : null;

  // PRIORIDAD: Imagen de producto (slider) > Imagen de categoría (helper) > Placeholder
  const displayImage = productThumb || categoryBaseImage;
    ...
    >
      ...
      <div className="relative h-60 w-full overflow-hidden bg-gray-100">
        <img
          src={displayImage}
          alt={category.name}
          className="h-full w-full object-cover transition-opacity duration-700 ease-in-out group-hover:scale-110"
          loading="lazy"
          onError={(e) => {
            // Si la imagen falla (incluso la del helper), ponemos el placeholder
            if (e.target.src !== window.location.origin + '/placeholder-category.jpg') {
              e.target.src = '/placeholder-category.jpg';
            }
          }}
        />
        
        {/* Overlay cuando pasas el mouse */}
