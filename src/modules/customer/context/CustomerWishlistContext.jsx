...
export const CustomerWishlistProvider = ({ children }) => {
   ...
   */
  const addItem = useCallback(async (productId, notifyPriceChange = false, notifyAvailability = false) => {
    try {
      setIsLoading(true);
      setError(null);
   ...
   */
  const togglePriceNotification = useCallback(async (productId, enabled) => {
    // Primero remover
...
export const useCustomerWishlist = () => {
  ...
  if (!context) {
    throw new Error('useCustomerWishlist must be used within CustomerWishlistProvider');
  }
  return context;
};
