...
import { SHIPPING_METHODS, SHIPPING_METHOD_LABELS, SHIPPING_COSTS } from '../domain/cart.constants';
 ...
 *
 * MIGRADO: ya no recibe props de estado ni callbacks.
 * Lee desde useCart() y escribe vía useCartActions().
 * UI idéntica al original.
 */
const ShippingForm = () => {
  // ── HOOKS ─────────────────────────────────────────────────────────────────
  const { shippingInfo, loading } = useCart();
  const { updateAddress, updateShipping } = useCartActions(
    (msg) => toast.success(String(msg)),
    (msg) => toast.error(typeof msg === 'string' ? msg : msg?.message || 'Error al guardar')
  );

  // ── ESTADO LOCAL ──────────────────────────────────────────────────────────
  const currentShipping = shippingInfo?.method;
  const currentAddress  = shippingInfo?.address;
  const isLoading       = loading.global;

  const [selectedMethod, setSelectedMethod] = useState(
    currentShipping || SHIPPING_METHODS.STANDARD
  );
  ...
  const handleMethodSelect = async (method) => {
    ...
    await updateShipping({ method, cost });
  };
  ...
  const handleSaveAddress = async () => {
    ...
    if (result?.success) setShowAddressForm(false);
  };
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        {currentAddress && !showAddressForm && (
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600 mt-1">{currentAddress.street}</p>
            <p className="text-sm text-gray-600">
              {currentAddress.city}, {currentAddress.state} {currentAddress.zipCode}
            </p>
            <p className="text-sm text-gray-600">{currentAddress.country}</p>
            <p className="text-sm text-gray-600 mt-2">{currentAddress.phone}</p>
            <p className="text-sm text-gray-600">{currentAddress.email}</p>
          </div>
        )}
