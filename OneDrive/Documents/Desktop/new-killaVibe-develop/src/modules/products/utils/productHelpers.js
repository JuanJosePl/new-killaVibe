...
export const isProductAvailable = (product) => {
  ...
  // Solo bloqueamos si el estado es explícitamente "no activo"
  ...
  // Si NO hay campo trackQuantity, o es false, el producto SIEMPRE está disponible
  if (product.trackQuantity === undefined || product.trackQuantity === false) {
    return true;
  }

  // Si se trackea cantidad, validar stock o pre-orden
...
export const getAvailabilityStatus = (product) => {
  ...
  // Si el backend no pide controlar stock, siempre está disponible
  if (product.trackQuantity === undefined || product.trackQuantity === false) {
    return 'available';
  }
  ...
  if (stock > threshold) return 'available';
  if (stock > 0) return 'low_stock';
  if (product.allowBackorder) return 'backorder';
